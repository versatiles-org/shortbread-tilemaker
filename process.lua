...
node_keys = {"place", "highway", "railway", "aeroway", "amenity", "aerialway", "shop", "leisure",
             "tourism", "man_made", "historic", "emergency", "office", "addr:housenumber",
             "addr:housename"}
...
poi_amenity_values = Set {"police", "fire_station", "post_box", "post_office", "telephone",
                          "library", "townhall", "courthouse", "prison", "embassy",
                          "community_centre", "nursing_home", "arts_centre", "grave_yard",
                          "marketplace", "recycling", "university", "school", "college",
                          "public_building", "pharmacy", "hospital", "clinic", "doctors", "dentist",
                          "veterinary", "theatre", "nightclub", "cinema", "restaurant", "fast_food",
                          "cafe", "pub", "bar", "foot_court", "biergarten", "shelter", "car_rental",
                          "car_wash", "car_sharing", "bicycle_rental", "vending_machine", "bank",
                          "atm", "toilets", "bench", "drinking_water", "fountain", "hunting_stand",
                          "waste_basket", "place_of_worship"}
catering_values = Set {"restaurant", "fast_food", "pub", "bar", "cafe"}
poi_leisure_values = Set {"playground", "dog_park", "sports_centre", "pitch", "swimming_pool",
                          "water_park", "golf_course", "stadium", "ice_rink"}
...
poi_shop_values = Set {"supermarket", "bakery", "kiosk", "mall", "department_store", "general",
                       "convenience", "clothes", "florist", "chemist", "books", "butcher", "shoes",
                       "alcohol", "beverages", "optician", "jewelry", "gift", "sports",
                       "stationery", "outdoor", "mobile_phone", "toys", "newsagent", "greengrocer",
                       "beauty", "video", "car", "bicycle", "doityourself", "hardware", "furniture",
                       "computer", "garden_centre", "hairdresser", "travel_agency", "laundry",
                       "dry_cleaning"}
poi_man_made_values = Set {"surveillance", "tower", "windmill", "lighthouse", "wastewater_plant",
                           "water_well", "watermill", "water_works"}
poi_historic_values = Set {"monument", "memorial", "artwork", "castle", "ruins",
                           "archaelogical_site", "wayside_cross", "wayside_shrine", "battlefield",
                           "fort"}
...
function fallback2(value1, value2)
   if value1 ~= "" then
      return value1
   end
   return value2
...
function fallback3(value1, value2, value3)
   if value1 ~= "" then
      return value1
   end
   if value2 ~= "" then
...
function setNameAttributes(obj)
   local attributes = getNameAttributes({
      name = obj:Find("name"),
      name_de = obj:Find("name:de"),
      name_en = obj:Find("name:en"),
      name_es = obj:Find("name:es"),
      name_fr = obj:Find("name:fr")
   })

   obj:Attribute("name_de", attributes.name_de)
   obj:Attribute("name_en", attributes.name_en)
   obj:Attribute("name_es", attributes.name_es)
   obj:Attribute("name_fr", attributes.name_fr)
   obj:Attribute("name", attributes.name)
end

-- Set name, name_en, and name_de on any object
function getNameAttributes(attr)
   -- Fill with fallback values if empty
   name = fallbackList({attr.name, attr.name_en, attr.name_es, attr.name_fr, attr.name_de});

   return {
      name = name,
      name_de = fallback2(attr.name_de, name),
      name_en = fallback2(attr.name_en, name),
      name_es = fallback2(attr.name_es, name),
      name_fr = fallback2(attr.name_fr, name)
   }
end
...
function setZOrder(way, is_rail, ignore_bridge)
   ...
   elseif is_rail and railway == "rail" then
      hwClass = 12
   elseif is_rail and
      (railway == "subway" or railway == "light_rail" or railway == "tram" or railway == "funicular" or
         railway == "monorail") then
      hwClass = 11
   ...
   elseif highway == "tertiary" then
      hwClass = 6
   elseif highway == "unclassified" or highway == "residential" or highway == "road" or highway ==
      "motorway_link" or highway == "trunk_link" or highway == "primary_link" or highway ==
      "secondary_link" or highway == "tertiary_link" or highway == "busway" or highway ==
      "bus_guideway" then
      hwClass = 5
   ...
   elseif highway == "service" then
      hwClass = 3
   elseif highway == "footway" or highway == "bridleway" or highway == "cycleway" or highway ==
      "path" or highway == "track" then
      hwClass = 2
...
function process_place_layer(node)
   ...
   if (place == "city" or place == "town" or place == "village" or place == "hamlet") and
      node:Holds("capital") then
...
function node_function(node)
   ...
   if railway == "station" or railway == "halt" or railway == "tram_stop" or highway == "bus_stop" or
      amenity == "bus_station" or amenity == "ferry_terminal" or aeroway == "aerodrome" or aeroway ==
      "helipad" or aerialway == "station" then
...
function zmin_for_area(way, min_square_pixels, way_area)
   ...
   local zmin = (math.log((min_square_pixels * circumfence ^ 2) / (2 ^ 16 * way_area))) /
                   (2 * math.log(2))
...
function process_water_polygons(way, way_area)
   ...
   if landuse == "reservoir" or landuse == "basin" or (natural == "water" and not is_river) or
      natural == "glacier" then
...
function process_land(way)
   ...
   if landuse == "forest" or natural == "wood" then
      kind = "forest"
      mz = 7
   elseif landuse == "residential" or landuse == "industrial" or landuse == "commercial" or landuse ==
      "retail" or landuse == "railway" or landuse == "landfill" or landuse == "brownfield" or
      landuse == "greenfield" or landuse == "farmyard" or landuse == "farmland" then
      kind = landuse
      mz = 10
   elseif landuse == "grass" or landuse == "meadow" or landuse == "orchard" or landuse == "vineyard" or
      landuse == "allotments" or landuse == "village_green" or landuse == "recreation_ground" or
      landuse == "greenhouse_horticulture" or landuse == "plant_nursery" or landuse == "quarry" then
      kind = landuse
      mz = 11
   elseif natural == "sand" or natural == "beach" then
      kind = natural
      mz = 10
   elseif natural == "wood" or natural == "heath" or natural == "scrub" or natural == "grassland" or
      natural == "bare_rock" or natural == "scree" or natural == "shingle" or natural == "sand" or
      natural == "beach" then
      kind = natural
      mz = 11
   elseif wetland == "swamp" or wetland == "bog" or wetland == "string_bog" or wetland ==
      "wet_meadow" or wetland == "marsh" then
      kind = wetland
      mz = 11
   ...
   elseif landuse == "garages" then
      kind = landuse
      mz = 12
   elseif leisure == "golf_course" or leisure == "park" or leisure == "garden" or leisure ==
      "playground" or leisure == "miniature_golf" then
      kind = leisure
      mz = 11
...
function process_sites(way)
   ...
   local landuse = way:Find("landuse")
   local mz = inf_zoom
   if amenity == "university" or amenity == "hospital" or amenity == "prison" or amenity ==
      "parking" or amenity == "bicycle_parking" or amenity == "school" or amenity == "college" then
      kind = amenity
      mz = 14
...
function toBridgeBool(bridge)
   if bridge == "yes" or bridge == "viaduct" or bridge == "boardwalk" or bridge == "cantilever" or
      bridge == "covered" or bridge == "low_water_crossing" or bridge == "movable" or bridge ==
      "trestle" then
      return true
   end
   return false
...
function process_streets(way)
   ...
   if highway ~= "" then
      ...
      elseif highway == "tertiary_link" then
         mz = 11
         kind = "tertiary"
      elseif highway == "unclassified" or highway == "residential" or highway == "bus_guideway" or
         highway == "busway" then
         mz = 12
      ...
      elseif highway == "service" then
         mz = 14
      elseif highway == "footway" or highway == "steps" or highway == "path" or highway ==
         "cycleway" then
         mz = 13
      end
   elseif (railway == "rail" or railway == "narrow_gauge") and service == "" then
      ...
      rail = true
      mz = 8
   elseif ((railway == "rail" or railway == "narrow_gauge") and service ~= "") or railway ==
      "light_rail" or railway == "tram" or railway == "subway" or railway == "funicular" or railway ==
      "monorail" then
      kind = railway
      rail = true
      mz = 10
   ...
   if kind ~= "" and surface ~= "" then
      if surface == "unpaved" or surface == "compacted" or surface == "dirt" or surface == "earth" or
         surface == "fine_gravel" or surface == "grass" or surface == "grass_paver" or surface ==
         "gravel" or surface == "ground" or surface == "mud" or surface == "pebblestone" or surface ==
         "salt" or surface == "woodchips" or surface == "clay" then
         suface = "unpaved"
      elseif surface == "paved" or surface == "asphalt" or surface == "cobblestone" or surface ==
         "cobblestone:flattended" or surface == "sett" or surface == "concrete" or surface ==
         "concrete:lanes" or surface == "concrete:plates" or surface == "paving_stones" then
         suface = "unpaved"
      else
         surface = ""
      end
   ...
   local link =
      (highway == "motorway_link" or highway == "trunk_link" or highway == "primary_link" or highway ==
         "secondary_link" or highway == "tertiary_link")
   local layer = tonumber(way:Find("layer"))
   if layer == nil then
      layer = 0
   end
...
function process_street_labels(way)
   ...
   elseif highway == "tertiary_link" then
      ...
      kind = "tertiary"
      link = true
   elseif highway == "unclassified" or highway == "residential" or highway == "busway" or highway ==
      "bus_guideway" or highway == "living_street" or highway == "pedestrian" or highway == "track" or
      highway == "service" or highway == "footway" or highway == "steps" or highway == "path" or
...
function process_aerialways(way)
   ...
   if aerialway == "cable_car" or aerialway == "gondola" or aerialway == "chair_lift" or aerialway ==
      "drag_lift" or aerialway == "t-bar" or aerialway == "j-bar" or aerialway == "platter" or
      aerialway == "rope_tow" then
...
function process_pois(obj, polygon)
   ...
   if amenity == nil and shop == nil and tourism == nil and historic == nil and leisure == nil and
      emergency == nil and highway == nil and office == nil then
      return false
   end
   if polygon then
...
function way_function(way)
   ...
   local area_yes_multi_boundary = (area_tag == "yes" or type_tag == "multipolygon" or type_tag ==
                                      "boundary" or area_aeroway_tag == "runway" or area_aeroway_tag ==
                                      "taxiway")
   ...
   -- Layer land
   if is_area and
      (way:Holds("landuse") or way:Holds("natural") or way:Holds("wetland") or way:Find("amenity") ==
         "grave_yard" or way:Holds("leisure")) then
      process_land(way)
   end

   -- Layer sites
   if is_area and
      (way:Holds("amenity") or way:Holds("leisure") or way:Holds("military") or way:Holds("landuse")) then
      process_sites(way)
   end
   ...
   if not is_area_default_linear and
      (way:Holds("highway") or way:Holds("railway") or way:Holds("aeroway")) then
      process_streets(way)
   ...
   local aeroway = way:Find("aeroway")
   if is_area and
      (railway == "station" or railway == "halt" or aeroway == "aerodrome" or aeroway == "helipad" or
         highway == "bus_stop" or amenity == "bus_station" or amenity == "ferry_terminal") then
...
function admin_level_valid(admin_level, is_unset_valid)
   return (is_unset_valid and admin_level == "") or admin_level == "2" or admin_level == "3" or
             admin_level == "4"
end
...
function attribute_function(attr, layer)
   ...
   if layer == "boundary_labels" then
      attributes = getNameAttributes(attr)

      attributes.admin_level = tonumber(attr.adm_level)
      attributes.way_area = attr.way_area
      attributes.land_area = attr.land_area
